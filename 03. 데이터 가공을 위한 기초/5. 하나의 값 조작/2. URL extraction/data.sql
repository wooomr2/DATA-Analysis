...
COMMENT ON COLUMN access_log.seq                       IS 'PK) seq';
COMMENT ON COLUMN access_log.user_id                   IS 'FK) user_id';
COMMENT ON COLUMN access_log.referer                   IS '어떤 웹 페이지를 거쳐 왔는지';
COMMENT ON COLUMN access_log.url                       IS '접속 URL';
